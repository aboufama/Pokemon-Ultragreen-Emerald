...
Read `src/render3d/arena/arenas.ts` first: the three places are Route 101's
`meadow`, the open sea's `sea()` and Granite Cave's `cave()`. Then `design.ts`
(the painting helpers) and the header of `src/render3d/environment.ts`.
...
| 25-60 | 15-9.5 | the back of the arena: paths, ledges, the wild Pokémon's surroundings |
...
rocks) is painted into the ground with `stand()` or `ground.sprite()`:
...
| layer | how | where |
|---|---|---|
| ground and far view | `fill()` paints every screen pixel from its ground point (x, z, ppu); `scatter()` spreads marks evenly for their distance; `stand()` paints trees and rocks in the far view (and painted foreground framing) with shadows; `onLine()` draws crisp 1-pixel pattern lines (wave crests, ripples) at any distance; `shafts()` light shafts (dithered, or `banded`); `shift()` moves a color along its ramp (lighten/darken in the palette); `foeCalm()` how deep a pixel is in the calm around the wild Pokémon; in arenas.ts `tint()` (haze) and `squeeze()` (a ramp drawn toward its middle: less contrast, same hues) | `design.ts`, `art.ts`, `arenas.ts` |
| geometry | a chamber with side walls and terraced ledges is traced per pixel into typed arrays once (the cave) | `arenas.ts` |
| materials | each painted pixel carries a material the ground shader animates per GBA pixel: `GRASS` (leans, wind bands), `WATER` (drifting waves, glints, ripples at the feet), `BACKDROP` (far things: no ground effects), `SOLID` | `art.ts` `MAT`, `ground.ts` |
| props | pixel-art sprites made at their on-screen size (`sprites.ts`: trees and bushes (`speckle`, `soft` in their palette), tall grass, reeds, rocks (`cracks`), faceted `crag`s, stalagmites) standing at their depth, hidden by and hiding the Pokémon by depth, swaying row by row in whole pixels | `props.ts`, `addProp()` / `frameProp()` |
...
7. **Readable behind the Pokémon.** Keep detail out of the calm around the
   ...
   pebbles, props or boulders there; a calm wall, water or a hazy
...
- **Around the wild Pokémon, nothing.** Every scatter of marks (tufts,
  clover, pebbles, hatching, ripples) skips pixels in
  `foeCalm()`; no props, boulders or reeds stand
...
- **Marks a shade off their ground, never two, and few.** Sparse scatters
  (the path's pebbles are 8% of cells), none a near-black
  or white dot on a light or dark ground.
- **Detail as long lines, not scattered dashes.** The sea's crests are long
  crisp runs; far off, fine repeated marks break into dotted rows, so keep
  them near.
- **Clean bands, little dither.** `band()` softness 0.05-0.1 on grounds and
  far views; a gradient steps in whole shades, never a wide dithered field. Light
...
Every arena as it was before this calm pass fails at least one gate. To see what a measure counts, map it: mark the pixels that count as
...
- **Crisp lines at any distance** with `onLine()`: wave crests, ripples; lines that crowd closer than `maxStep` drop out (keep every
...
- **Framing** a dark rock or tuft cluster painted at the left edge of the
...
| rocks that read as eggs or beans | faceted `crag()` for sea rock |
| far reeds that read as posts | thin ribbons, fewer, in clumps, one flat color far off |
